...
class AuthService:
    ...
    def signup(self, db: Session, data: UserCreate):
        with db.begin():
            # 1. 이메일 중복 검사
            existing_user = user_repository.find_by_student_number(db, data.student_number)
            if existing_user:
                raise HTTPException(
            ...
            # 3. 사용자 저장
            new_user = User(student_number=data.student_number, password=hashed_password, name=data.name)
    ...
    def login(self, db: Session, data: UserLogin) -> str:
        # 1. 이메일로 사용자 조회
        user = user_repository.find_by_student_number(db, data.student_number)
        if not user:
            raise HTTPException(
